    ...
    dateClick: function (info) {
        ...
        if (selectedDate < today) {
            ...
            if (slotContainer.find('.no-availability-text').length === 0) {
                slotContainer.append('<p class="no-availability-text">Дата уже прошла.</p>');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (nextAvailableDateSelector.length > 0) {
                ...
                const nextDateText = `<p class="next-available-date">Следующая доступная дата: <br>${formattedDate}</p>`;
